...
def version_saving(vectorizer, labencoder, model, new_model_history):
    ...
    hist_df = pd.read_csv("versions\\model\\model_training_history.csv",index_col = 0)
    ...
    CV_filename = "versions\\count_vectorizer\\Rakuten_CountVectorizer_v{}.sav".format(new_version)
    joblib.dump(vectorizer, CV_filename)
    #Sauvegarde du label_encoder
    LE_filename = "versions\\label_encoder\\Rakuten_LabelEncoder_v{}.sav".format(new_version)
    joblib.dump(labencoder, LE_filename)
    #sauvegarde du model
    model_directoryname = "versions\\model\\Rakuten_model_v{}".format(new_version)

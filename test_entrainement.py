...
def test_save_CV():
    assert os.path.exists("versions\\count_vectorizer\\Rakuten_CountVectorizer_v{}.sav".format(new_version)) ==True
...
def test_save_LE():
    assert os.path.exists("versions\\label_encoder\\Rakuten_LabelEncoder_v{}.sav".format(new_version)) == True
...
def test_save_model():
    assert os.path.exists("versions\\model\\Rakuten_model_v{}".format(new_version))
